...
const upload = multer({
  ...
  limits: {
    fileSize: 25 * 1024 * 1024 // 25MB limit for uploads
  }
});
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/user/profile", isAuthenticated, express.json({limit: '1mb'}), async (req, res) => {
    try {
      const userId = req.user!.id;
      const { name, bio, work, education, currentCity, hometown } = req.body;

      // Update user info without images
